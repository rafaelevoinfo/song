...
uses
  ...
  (*IBDatabase, IBTable, *)ComCtrls(*, Cripto(*, DateUtil*),
  inifiles, FileCtrl;
...
begin
  if ipGravarLog then
    begin
      ...
      if DirectoryExists('C:\Temp') then
        if not(DirectoryExists('C:\Temp\SoftDataLog')) then
          if not CreateDir('C:\Temp\SoftDataLog') then
            raise Exception.Create('Problemas para criar a pasta C:\Temp\SoftDataLog');
      if DirectoryExists('c:\Temp\SoftDataLog') then
        begin
          try
            AssignFile(F, 'C:\Temp\SoftDataLog\' + 'Log' + Copy(DateToStr(Date),7,4) + copy(DateToStr(Date),4,2) + copy(DateToStr(Date),1,2) + '.log');
            if not FileExists('C:\Temp\SoftDataLog\' + 'Log' + Copy(DateToStr(Date),7,4) + copy(DateToStr(Date),4,2) + copy(DateToStr(Date),1,2) + '.log') then
              ReWrite(F)
            else
              Append(F);
  ...
  Form_SimNao.Memo1.Text := #13#10 + ipMensagem;
  Form_SimNao.btnOK.Visible := not( ipTipo_Confirmacao );
  if ipTipo_Confirmacao then
    Form_SimNao.Caption := 'Confirmação'
  else
    Form_SimNao.Caption := 'Atenção';
  if Pos('Unable to complete network request to host', Form_SimNao.Memo1.Text) > 0 then
    Form_SimNao.Memo1.Text := 'A REDE NÃO ESTA FUNCIONANDO CORRETAMENTE.'+#13+#10+#13+#10+
                              Form_SimNao.Memo1.Text;
  Form_SimNao.Timer1.Interval := ipTempo;
  Form_SimNao.ShowModal;
  Result := (Form_SimNao.ComboBox1.ItemIndex = 1);
  Form_SimNao.Free;
end;
